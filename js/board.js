...
function renderRoomScene(roomId, gameState, onInspectClue, onWipeClue) {
  ...
  if (btnJigsaw) {
    const isActionUsed = !!(gameState.turnMoved || gameState.turnClueActionDone || gameState.turnMiniGamePlayed);
    btnJigsaw.innerHTML = '🧩 ถอดรหัสจิ๊กซอว์';

    if (isActionUsed) {
      btnJigsaw.classList.add('btn-jigsaw-disabled');
      btnJigsaw.title = gameState.turnMoved
        ? 'เพิ่งเดินเข้าห้องมา — สามารถเล่นมินิเกมส์ได้ในเทิร์นถัดไป'
        : 'ใช้การกระทำในเทิร์นนี้แล้ว (จำกัด 1 แอคชั่น/เทิร์น)';
    } else {
    ...
    freshJigsaw.onclick = () => {
      if (gameState.turnMoved) {
        addLog('คุณเพิ่งเดินเข้าห้องมาในเทิร์นนี้ จึงยังไม่สามารถเล่นมินิเกมส์ได้ กรุณากดจบเทิร์น', 'danger');
        return;
      }
  ...
  const isCluesDisabled = !!(gameState.turnMoved || gameState.turnClueActionDone);
  ...
  clues.forEach(clue => {
    ...
    if (isCluesDisabled) {
      ...
      marker.title = gameState.turnMoved
        ? 'เดินเข้าห้องแล้ว — สามารถตรวจร่องรอยได้ในเทิร์นถัดไป'
        : 'ตรวจร่องรอยในเทิร์นนี้แล้ว (จำกัด 1 จุดต่อเทิร์น)';
    ...
    if (clues.length === 0) {
      ...
      clues.forEach((clue, idx) => {
        ...
        if (isCluesDisabled) {
          ...
          if (gameState.turnMoved) {
            statusText = '🚶 เดินเข้าห้องแล้ว (ตรวจได้ในเทิร์นถัดไป)';
          } else if (gameState.turnClueActionDone) {
