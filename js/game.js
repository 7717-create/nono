...
function startTurn() {
  ...
  if (!player.currentRoomId) {
    ...
    updateGuideBanner('🗺️', `<strong>ขั้นตอนที่ 1 (ตาของ ${player.name}):</strong> คลิกเลือกห้อง 1 ห้องบนแผนผังคฤหาสน์เพื่อเดินเข้า (เมื่อเข้าห้องแล้วจะจบเทิร์นทันที)`);
  } else {
    // If player is already inside a room (including 'hall'!): show Room Scene View with ? markers and buttons
    updateTurnStepBar(2);
    showRoomScene();
    renderRoomScene(player.currentRoomId, G, (clue) => doInspectClue(clue), (clue) => doWipeClue(clue));
    
    if (player.isKiller && !player.isAI) {
      updateGuideBanner('🩸', `<strong>ขั้นตอนที่ 2 (อยู่ใน${curRoom.name}):</strong> คุณคือฆาตกร! เลือกตรวจหรือลบหลักฐาน <strong>1 จุดเท่านั้นต่อเทิร์น</strong> (หรือคลิก "สลับดูแผนผัง" เพื่อย้ายห้อง แต่จะจบเทิร์นทันที)`);
    } else {
      updateGuideBanner('🔍', `<strong>ขั้นตอนที่ 2 (อยู่ใน${curRoom.name}):</strong> ตรวจร่องรอยในห้องนี้ได้ <strong>1 จุดเท่านั้นต่อเทิร์น</strong> (หรือคลิก "สลับดูแผนผัง" เพื่อย้ายห้อง แต่จะจบเทิร์นทันที)`);
    }
  }
...
function doSelectRoom(roomId) {
  ...
  const room = MANSION_ROOMS[roomId];
  addLog(`${player.name} ${isMove ? 'ย้ายไปที่' : 'เดินเข้าสู่'} ${room.emoji} ${room.name} (จบการเดินในเทิร์นนี้)`, 'highlight');

  // Moving rooms ends clue actions for this turn — jump to Step 3: End Turn
  updateTurnStepBar(3);
  showRoomScene();
  renderRoomScene(roomId, G, (clue) => doInspectClue(clue), (clue) => doWipeClue(clue));
  renderPlayersList(G.players, G.currentPlayerIndex);
  ...
  const actionLabel = isMove ? '🚶 ย้ายห้องแล้ว ➔ จบเทิร์น' : '🚶 เข้าห้องแล้ว ➔ จบเทิร์น';
  setEndTurnBtnState(true, nextP ? nextP.name : '', actionLabel);

  updateGuideBanner(
    '🚶',
    `<strong>${isMove ? 'ย้ายห้อง' : 'เข้าห้อง'}สำเร็จ (${room.name}):</strong> คุณได้ใช้การเดินในเทิร์นนี้แล้ว ไม่สามารถตรวจร่องรอยได้ กรุณากดปุ่ม <strong>"จบเทิร์น ➔"</strong> เพื่อส่งต่อเทิร์น`
  );
...
function doInspectClue(clue) {
  ...
  if (G.turnMoved) {
    addLog('คุณเพิ่งเดินเข้าห้องมาในเทิร์นนี้ จึงยังไม่สามารถตรวจร่องรอยได้ กรุณากดจบเทิร์น', 'danger');
    return;
  }
...
function doSwapClue(originalClue, forgedClue) {
  ...
  if (G.turnMoved) {
    addLog('คุณเพิ่งเดินเข้าห้องมาในเทิร์นนี้ จึงยังไม่สามารถสลับหลักฐานได้ กรุณากดจบเทิร์น', 'danger');
    return;
  }
  if (G.turnClueActionDone) {
  ...
  const clueTypeText = forgedClue.isCritical ? 'เบาะแสสำคัญลวง (ป้ายความผิด)' : 'เบาะแสลวงเฉยๆ';
  addLog(`🎭 ${player.name} สลับหลักฐานเป็น "${forgedClue.title}" (${clueTypeText}) ใน ${room ? room.name : ''}!`, 'highlight');
  ...
  updateGuideBanner('🎭', `<strong>ขั้นตอนที่ 3:</strong> สลับเป็น${clueTypeText}สำเร็จ! จุดนี้เปลี่ยนเป็น <strong>"?"</strong> เรียบร้อย และข้อมูลถูกบันทึกลงแฟ้ม กรุณากดปุ่ม <strong>"จบเทิร์น ➔"</strong> ด้านขวา`);
}
...
function doWipeClue(clue) {
  ...
  if (G.turnMoved) {
    addLog('คุณเพิ่งเดินเข้าห้องมาในเทิร์นนี้ จึงยังไม่สามารถทำลายร่องรอยได้ กรุณากดจบเทิร์น', 'danger');
    return;
  }
  if (G.turnClueActionDone) {
    addLog('คุณได้ดำเนินการกับร่องรอยในเทิร์นนี้แล้ว (จำกัด 1 จุดต่อ 1 เทิร์น) กรุณากดจบเทิร์น', 'danger');
...
function bindGameButtons() {
  ...
  if (btnBackToMap) {
    btnBackToMap.onclick = () => {
      if (G.turnClueActionDone || G.turnMoved || G.turnMiniGamePlayed) {
        addLog('คุณได้ดำเนินการในเทิร์นนี้แล้ว ไม่สามารถย้ายห้องได้ กรุณากดจบเทิร์น', 'danger');
        return;
      }
      showMansionMap();
      renderMansionMap(G, (roomId) => doSelectRoom(roomId));
      updateGuideBanner('🗺️', `เลือกห้อง 1 ห้องเพื่อย้ายเข้าไป (เมื่อย้ายห้องแล้วจะจบเทิร์นทันที)`);
    };
  }
